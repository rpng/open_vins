...
double FeatureInitializer::compute_error(std::unordered_map<size_t,std::unordered_map<double,clonePose>> &clonesCAM,Feature* feat,double alpha,double beta,double rho){
    // Total error
    double err = 0;
...
bool FeatureInitializer::single_gaussnewton(Feature* feat, std::unordered_map<size_t,std::unordered_map<double,clonePose>> &clonesCAM) {
    ...
    feat->p_invFinA(0,0)= alpha;
    feat->p_invFinA(1,0)= beta;
    feat->p_invFinA(2,0)= rho;
    ...
    // 3. If the baseline ratio is large (todo: look into if 50 works best)
