...
void VioManagerCalib::feed_measurement_simulation(double timestamp, const std::vector<int> &camids, const std::vector<std::vector<std::pair<size_t,Eigen::VectorXf>>> &feats) {
    ...
    if(trackArr == nullptr) {
        cv::Mat cameraMatrix = (cv::Mat_<double>(3, 3) << 458.654, 0.0, 367.215,
                                                      0.0, 457.296, 248.375,
                                                        0.0, 0.0, 1.0);
        trackArr = new TrackArr();
        trackArr->feed_camera_matrix(cameraMatrix);
        printf(RED "[SIM]: assigning our TrackArr object!\n" RESET);
    }
    ...
    // TODO: add trackArr here
    trackArr->push_back(timestamp, feats);
    trackArr->calc_motion(angvel, angacc, R);
