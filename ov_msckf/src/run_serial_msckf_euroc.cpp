...
int main(int argc, char** argv)
{
    ...
    // Load the config
    auto parser = std::make_shared<ov_core::YamlParser>(config_file_path);
    ...
    parser->parse_config("dataset_path", euroc_dataset_path, true);
    parser->parse_config("gt_file_path", gt_file_path, false);
    parser->parse_config("output_pose_file_path", output_pose_file_path, false);
    parser->parse_config("t_start", t_start, false);
    parser->parse_config("t_end", t_end, false);
    ...
    for(size_t idx_frame = idx_start; idx_frame < idx_end; idx_frame++) {
        ...
        if(!gt_states.empty() && !sys->initialized() && get_gt_state(t_img, imustate, gt_states)) {
            ...
            if(params.state_options.num_cameras == 1) {
                ...
                measure_img.masks.push_back(cv::Mat::zeros(img_left.size(), CV_8UC1));
                ...
                measure_img.masks.push_back(cv::Mat::zeros(img_right.size(), CV_8UC1));
                sys->feed_measurement_camera(measure_img);
            }
        }
