...
void StateHelper::convert_msckfslam_to_singledepth(State *state, Landmark*& landmark) {

    // assert that this feature representation is the inverse depth msckf
    if(landmark->_feat_representation!=LandmarkRepresentation::Representation::ANCHORED_MSCKF_INVERSE_DEPTH) {
        printf(RED "StateHelper::convert_msckfslam_to_singledepth() - Trying to convert non-anchored msckf feature to single depth\n" RESET);
        printf(RED "StateHelper::convert_msckfslam_to_singledepth() - One can only convert ANCHORED_MSCKF_INVERSE_DEPTH features!!\n" RESET);
        std::exit(EXIT_FAILURE);
    }

    // our bearing is just the first two of this feature, while the last is rho
    // by setting the value, this will correctly set the bearing for this feature
    Landmark* landmark_new = new Landmark(1);
    landmark_new->set_local_id(landmark->id());
    landmark_new->_featid = landmark->_featid;
    landmark_new->_feat_representation = LandmarkRepresentation::Representation::ANCHORED_INVERSE_DEPTH_SINGLE;
    landmark_new->_anchor_cam_id = landmark->_anchor_cam_id;
    landmark_new->_anchor_clone_timestamp = landmark->_anchor_clone_timestamp;
    landmark_new->set_from_xyz(landmark->get_xyz(false), false);
    landmark_new->set_from_xyz(landmark->get_xyz(true), true);

    // Marginalize the first two of the mskcf feature
    // These corresponding to the bearing of the feature
    int marg_size = 2;
    int marg_id = landmark->id();
    ...
    for (size_t i = 0; i < state->_variables.size(); i++) {
        // Only keep non-marginal states
        if (state->_variables.at(i) != landmark) {
            // If the variable is "beyond" the marginal one in ordering, need to "move it forward"
            if (state->_variables.at(i)->id() > marg_id) {
                state->_variables.at(i)->set_local_id(state->_variables.at(i)->id()-marg_size);
            }
            ...
            remaining_variables.push_back(landmark_new);
        }
    }
    ...
    /// Set it equal to our state, and variable we pass back
    state->_variables = remaining_variables;
