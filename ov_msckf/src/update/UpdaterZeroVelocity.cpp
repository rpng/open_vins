...
bool UpdaterZeroVelocity::try_update(std::shared_ptr<State> state, double timestamp) {
  ...
  for (size_t i = 0; i < imu_recent.size() - 1; i++) {
    ...
    R.block(6 * i + 0, 6 * i + 0, 3, 3) *= _imu_config.sigma_w_2 / dt;
    if (!integrated_accel_constraint) {
      R.block(6 * i + 3, 6 * i + 3, 3, 3) *= _imu_config.sigma_a_2 / dt;
    } else {
      R.block(6 * i + 3, 6 * i + 3, 3, 3) *= _imu_config.sigma_a_2 * dt;
    }
    dt_summed += dt;
  ...
  Q_bias.block(0, 0, 3, 3) *= dt_summed * _imu_config.sigma_wb;
  Q_bias.block(3, 3, 3, 3) *= dt_summed * _imu_config.sigma_ab;
